...
obi export --tab-output --header /Volumes/Elements/EMBL_dms/COI_BF3_BR1_clean_annotated > ~/Documents/01_Work/01_Research/15_eDNA/Data/COI_refDB.txt
...
#remove insect as they are in BOLD_all_art
...
merge with embl seqs and export
...
```{bash}
/Applications/vsearch/bin/vsearch --threads 4 --cluster_fast /Volumes/Elements/BLAST/COI_full.fasta --centroids /Volumes/Elements/BLAST/COI_full_99.fasta --id 0.99 --uc /Volumes/Elements/BLAST/COI_full_99.txt
```
...
COI_full_clean <- clean(COI_full_99)

writeXStringSet(COI_full_clean, "/Volumes/Elements/BLAST/COI_full_99_clean.fasta")
```


## predict phylum
```{bash}
alfie -f /Volumes/Elements/BLAST/COI_full_99_clean.fasta
```


```{r}
alfi_files <- list.files("alfie_out") 
alfi_files <- alfi_files[grep( "COI_full_99_clean.fasta", alfi_files)]

phylum_pred <- tibble(seq = character(),
                      phylum = character())

for(i in seq_along(alfi_files)){
  phyl_fast <- readDNAStringSet(here("alfie_out", alfi_files[i]))
  phylum_temp <- tibble(seq = names(phyl_fast),
                        phylum = strsplit(alfi_files[i], "_")[[1]][1])
  phylum_pred <- bind_rows(phylum_pred,phylum_temp)
}
...
We now have 4 files: 
 ...
 (3) the two metadata files giving us the taxonomic annotation (from EMBL and BOLD) off all sequences (the centroids and the ones that have been clustered)
 (4) the predicted phylum for each centroid from the alfie package
 
 We read in all 4 files 
...
refDB_COI <- read_tsv("~/Documents/01_Work/01_Research/15_eDNA/Data/COI_refDB.txt")
...
refDB_COI <- filter(refDB_COI, ID %in% unique(c(Cluster_ID_COI$seq, Cluster_ID_COI$match)))
...
refDB_COI <- 
  refDB_COI %>% 
  mutate(db = "embl") %>% 
  bind_rows(refDB_COI, mutate(BOLD_tax, db = "bold"))
```

because the taxonomic annotation is not alwas coherent between the embl and the bold database, we use the gbif api to resolve synonymes and get a coherent taxonomic annotation from all species
...
gbif_specnames_join <-  
  gbif_specnames %>% 
  select(scientificName, rank, status, confidence, kingdom, phylum, class, order, family, genus, species)
...
```{bash}
blastn -num_threads 4 -query ~/Documents/01_Work/01_Research/15_eDNA/Data/COI_clustered_99.fasta -db /Volumes/Elements/BLAST/BOLD_merged_ref_derep.fasta -max_target_seqs 20 -out ~/Documents/01_Work/01_Research/15_eDNA/Data/BOLD_merged_blast.txt -outfmt '6 qseqid sseqid pident qstart qend  bitscore length  nident'  
```
...
```{r}
my_COI <- readDNAStringSet("~/Documents/01_Work/01_Research/15_eDNA/Data/COI_clustered_99.fasta")
...
```{bash}
blastn -query ~/Documents/01_Work/01_Research/15_eDNA/Data/COI_clustered_99.fasta -db /Volumes/Elements/BLAST/all_COI.fasta -max_target_seqs 20 -out ~/Documents/01_Work/01_Research/15_eDNA/Data/COI_blast_embl.txt -outfmt '6 qseqid sseqid pident qstart qend  bitscore length  nident'  
```
...
COI_seqs <- readDNAStringSet("~/Documents/01_Work/01_Research/15_eDNA/Data/COI_clustered_99.fasta")
