...
See Embl_DB.Rmd for details. 

It the extracts all seq that match the foward and the reverse primer, with 2 missmatches allowed. Note that it doensn't just grep fro the primers but simulates and *in-silico* PCR and returns the sequences that would be amplified. 
...
We now have 3 files: 
 ...
 (3) the metadata file giving us the taxonomic annotation (from EMBL) off all sequences (the centroids and the ones that have been clustered)
...
Of those, 253 have incongruences at the family level, 1513 have incongruences at the genus level and 9781 have incongruencies at the species level. 

Incongruencies can arise in different ways: 

(1) 16S is a very conserved marker. It is likely that clustering at 99% will create clusters genuinely have sequences from different species and even genera (see e.g. "AB034824")

(2) The are a lot of variation for species names were some sequences are only identified as sp. of cf. or have a number code / specimen id. This is likely the biggest culprit for species incongruencies.

(3) misslabeled / missidentified sequences. These are hard(er) to detect if they missidentification is at species or genus level but if sequences disagree at family level it is clear that some sequences in the cluster are misslabeled. 
...
(1) filter out clusters without consensus taxonomy (family deisagreemnt)
...
As a qualilty control, we blast the databas against itself 

+ make blast refdb

```{bash}
makeblastdb -in /Volumes/Elements/BLAST/16S_refDB_99_wtax.fasta -input_type 'fasta' -dbtype 'nucl'
```
...
(local mac)
```{bash}
blastn -num_threads 4 -query /Volumes/Elements/BLAST/16S_refDB_99_wtax.fasta -db /Volumes/Elements/BLAST/16S_refDB_99_wtax.fasta -max_target_seqs 20 -out /Volumes/Elements/BLAST/16S_refDB_self_blast.txt -outfmt '6 qseqid sseqid pident qstart qend  bitscore length  nident'  
```

(on remote windows machine)
```{bash}
blastn -num_threads 8 -query C:\Users\Fabian\Documents\Fabian\blast\16S_refDB_99_wtax.fasta -db C:\Users\Fabian\Documents\Fabian\blast\16S_refDB_99_wtax.fasta -max_target_seqs 20 -out C:\Users\Fabian\Documents\Fabian\blast\16S_refDB_99_self.txt -outfmt "6 qseqid sseqid pident qstart qend  bitscore length  nident""  

```

read in
```{r}
selfbalst <- read_tsv(here("Data", "16S_refDB_99_self.txt"), col_names = FALSE)
  
Blast_names <- c("ASV", "matchID", "pident","qstart", "qend","bitscore","length","nident")

colnames(selfbalst) <- Blast_names
...
selfbalst %>% 
  group_by(ASV) %>% 
  filter(bitscore > 0.9*max(bitscore)) %>% 
  mutate(n = n()) %>% 
  ungroup() %>% 
  filter(n > 1) %>% 
  select(1, 3:6) %>% 
...
refDB_16S %>% 
  filter(ID %in% c("MG253276","MG253279"))
```
...
```{r}
EMBL_blast_16s <- read_tsv("~/Documents/01_Work/01_Research/15_eDNA_pilot/Data/EMBL_blast_16S.txt", 
                           col_names = FALSE)
```
...
refDB_16S <- 
  refDB_16S %>% 
  select(ID, seq_length, forward_error, reverse_error, ends_with("_name")) %>% 
  mutate(seqID = paste("seq", formatC(1:n(), flag = "0",
                                     width = nchar(n()))))

refDB_16S <- 
  refDB_16S %>% 
  select(seqID, matches(".") )

refDB_16S <- 
  refDB_16S %>% 
  rename_at(vars(ends_with("_name")), function(x) gsub("_name", "", x))


```

many species are represented with 100s to 1000s of sequences
we keep the first entry for each species

'######### what about sequences with opposing taxonomic annotation? #######'
...
refDB_16S %>% mutate(char = nchar(ID)) %>%  filter(char == 13) 
```
...
refDB_16S %>% 
  ...
  summarise(n = n()) %>% 
  ggplot(aes(x = superkingdom, y = n, fill = superkingdom))+
...
```{r}
refDB_16S %>% 
  mutate(kingdom = case_when(is.na(kingdom) ~ superkingdom,
                                  TRUE ~ kingdom)) %>% 
  group_by(phylum) %>% 
  summarise(n = n(), kingdom = unique(kingdom)) %>% 
  arrange(desc(n)) %>% 
  mutate(phylum = factor(phylum, levels = .$phylum)) %>% 
  ggplot(aes(x = phylum, y = n, fill = kingdom))+
  geom_bar(stat = "identity")+
  scale_fill_brewer(palette = "Set1")+
  theme_bw()+
  scale_y_log10()+
...
'########## what about confilicting taxonomic assignment? ##########'

```{bash}

/Applications/vsearch/bin/vsearch --derep_fulllength /Volumes/Elements/BLAST/16S_refDB.fasta --output /Volumes/Elements/BLAST/16S_refDB_derep.fasta

```

#read fasta
```{r}
refseq16S <- readDNAStringSet("/Volumes/Elements/BLAST/16S_refDB_derep.fasta")

tax_derep <- taxdf[match(names(refseq16S), taxdf$id),]

tax_derep <- 
tax_derep %>% 
  select(-superkingdom, -scientific)

write_tsv(tax_derep, here("Data", "taxcode_16S.txt"))

names(refseq16S) <- apply(tax_derep, 1, paste0, collapse=";")

writeXStringSet(refseq16S, "/Volumes/Elements/BLAST/16S_refDB_derep.fasta")
```
...
tibble(id = gsub("(.?);.+", "\\1", names(refseq16S)),
  seqlen = width(refseq16S)) %>% 
  left_join(tax_derep) %>% 
#  pull(phylum) %>% unique
  filter(phylum %in% c("Arthropoda", "Chordata", "Mollusca", "Cnidaria", "Annelida")) %>% 
  #filter(phylum == "Chordata") %>% 
  ggplot(aes(x = seqlen, fill = phylum))+
  geom_histogram()+
  facet_wrap(~phylum)+
...
```{bash}
makeblastdb -in /Volumes/Elements/BLAST/16S_refDB_derep.fasta -input_type 'fasta' -dbtype 'nucl'
```

+ blastn against db

```{bash}
blastn -num_threads 4 -query ~/Documents/01_Work/01_Research/15_eDNA/Data/16S_clustered_99.fasta -db /Volumes/Elements/BLAST/16S_refDB_derep.fasta -max_target_seqs 20 -out ~/Documents/01_Work/01_Research/15_eDNA/Data/EMBL_blast_16S.txt -outfmt '6 qseqid sseqid pident qstart qend  bitscore length  nident'  
```

#blast results

```{r}
EMBL_blast_16s <- read_tsv("~/Documents/01_Work/01_Research/15_eDNA/Data/EMBL_blast_16S.txt", 
                           col_names = FALSE)
```
...
taxdf %>% filter(id == "KX467429")

```

```{r}
seq16S <- readDNAStringSet("~/Documents/01_Work/01_Research/15_eDNA/Data/16S_clustered_99.fasta")
```
