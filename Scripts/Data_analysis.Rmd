...
# sample meta-data
sampleInfo <- read_tsv(url("https://figshare.com/ndownloader/files/34001870")) 
...
16S: 
the blank sample contains 9 sequences for 16S. The three sequences identified as *Thrips flavidulus* are human sequences accounting for 84088 (86%) of the reads. Among teh other reads, only one has a taxonomic assignment that is trustworthy, *Sus scrofa*, which is the wild boar / domestic pig and could be a reagent contamination or a contamination from the sterile sampling location as it only occurs in that sample. Seq_0009 is very likely the same Caradina contamination as it also occurs in (almost) all samples
...
sum(ASV_COI[,humans_coi]) / sum(ASV_COI) *100
sum(ASV_16S[,humans_16S]) / sum(ASV_16S) *100
```
...
sum(ASV_COI[,contam_coi]) / sum(ASV_COI) *100
```
...
sum(ASV_COI[,fly_COI]) / sum(ASV_COI) *100
sum(ASV_16S[,fly_16S]) / sum(ASV_16S) *100
```
...
''''''''''''''''
(this code does not need to be run to continue with the script. To un the remainder of the script jump to ## consensus taxonomy)
''''''''''''''''
...
COI_ncbi_u[COI_ncbi_u$query == "seq_1716",]$name <- "Pelophylax ridibundus"#this a manual correction as this sequences matches both Pelophylax ridibundus and P. lessonae but the former is common and abundant in a pond near the detection site but the latter is rare and absent from that site.
...
tax_COI_clean %>% 
  ...
  filter(is.na(species.x) != is.na(species.y) | species.x != species.y) 
```

The assignemnt over ncbi Blast yields only one conflicting assignment, the manually renamed pool frog Pelophylax ridibundus (see above). However, there are some assignment that are missing a species level assigments in one of the two dataframes. We find a consensus manually. 

seq_1716
is Pelophylax ridibundus
...
#change pool frog to edible frog
tax_COI_clean[tax_COI_clean$seq == "seq_1716", "species"] <- "Pelophylax ridibundus"
```
...
## consensus taxonomy
```{r}
#read ncbi_16S_u_tax from figshare
ncbi_16S_u_tax <- read_tsv(url("https://figshare.com/ndownloader/files/33897374"))
...
ggsave(here("Figures", "Taxa_distribution.pdf"), width = 112, height = 160, units = "mm")
```
...
ggsave(filename = here("Figures", "Euler_combined.pdf"),
...
ggsave(here("Figures", "Bubble_combined_Lep_Hym.pdf"), height = 237, width = 169, units = "mm", plot = LepHym)

ggsave(here("Figures", "Bubble_combined_other.pdf"), height = 237, width = 169, units = "mm", plot = Other)
...
ggsave(here("Figures", "Specaccum.pdf"), width = 80, height = 100, units = "mm")
...
ggsave(here("Figures", "NMDS_COI.pdf"), width = 80, height = 100, units = "mm")
...
tax_16S_clean %>% pull(phylum) %>% unique %>% length
```
